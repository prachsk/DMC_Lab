...
lh.se <- CreateSeuratObject(lh.data, project = "LHA_scRNA", row.names = lh.nm)
lh.se

#Skip Mt genes QC as there is none
# Visualize QC metrics as a violin plot
VlnPlot(lh.se, features = c("nFeature_RNA", "nCount_RNA"), ncol = 2)
plot1 <- FeatureScatter(lh.se, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1

#Subset the data
lh.se <- subset(lh.se, subset = nFeature_RNA > 2000)
...
lh.se <- FindClusters(lh.se, resolution = 1)
