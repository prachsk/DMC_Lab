...
cell.data <- read.delim(file = '/Users/pax/Google\ Drive/My\ Drive/Meletis/Data_reproduce/GSE87544/GSE87544_Merged_17samples_14437cells_count.txt', header = TRUE, sep = '\t')
...
cell.se[["source"]] <- rep(c("Chen"), times = ncol(cell.se))


#Look for mitochodrial genes in the count matrix
cell.se[["percent.mt"]] <- PercentageFeatureSet(cell.se, pattern = "^Mt.")
VlnPlot(cell.se, features = c('nFeature_RNA', 'nCount_RNA', 'percent.mt'), ncol = 3)

# Select cells with at least 2000 genes
cell.se <- subset(cell.se, subset = nFeature_RNA > 2000)

# Find high variable genes
cell.se <- FindVariableFeatures(cell.se, nfeatures = 2000)
...
# Run PCA
cell.se <- RunPCA(cell.se, npcs = 100, features = VariableFeatures(object = cell.se))
VizDimLoadings(cell.se, dims = 1:2, reduction = "pca")
cell.se <- JackStraw(cell.se, dims = 100, num.replicate = 100)
cell.se <- ScoreJackStraw(cell.se, dims = 1:100)
JackStrawPlot(cell.se, dims = 1:100)
ElbowPlot(cell.se, ndims = 100)

# Find cluster
cell.se <- FindNeighbors(cell.se, dims = 1:58)
cell.se <- FindClusters(cell.se, resolution = 0.5)

# Run tsne
cell.se <- RunTSNE(cell.se, dims = 1:58)
DimPlot(cell.se, reduction = "tsne")

# Save Data
saveRDS(cell.se, "../RDS_obj/Chen_merge.RDS")
...
iter1.se <- subset(cell.se, idents = 1)
